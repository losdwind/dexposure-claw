...
| 训练 | 需实现 7个损失分量 | ✅ 已实现 |
| 训练 | 需实现 TVL 经济加权 | 🔴 待实现 |
| 实验 | 保留 h ∈ {1,3,7,14} 窗口 | ✅ 已实现 |

### 论文核心评估任务

| 任务 | 描述 | 状态 |
|------|------|------|
| Task I | Multi-step Forecasting (h=1,3,7,14) | ✅ 已实现 |
| Task II | Financial Stability Analysis | ⏳ 部分实现 |
| ├─ II.1 | Systemic Risk Measurement (SIS, Spillover) | ✅ 已实现 |
| ├─ II.2 | Shock Event Analysis (Terra/FTX) | ✅ 已实现 |
| └─ II.3 | Contagion Simulation (DebtRank) | ✅ 已实现 |
...
| 优先级 | 任务 | 说明 | 状态 |
|--------|------|------|------|
| ✅ 高 | 7个损失分量 | L_edge, L_link, L_node, L_stats, L_impute, L_scen, L_smooth | ✅ 已实现 |
| 🔴 高 | TVL 经济加权 | $w_{ij} \propto TVL_i \cdot E_{ij}$ | ⬜ 待实现 |
| ✅ 高 | h=14 预测窗口 | 添加两周预测实验 | ✅ 已实现 |
...
2. **TVL 经济加权**: 实现经济重要性加权

**🟡 中优先级:**
3. **Naive Baseline**: 验证模型优于简单启发式
4. **Shock Analysis**: Terra/FTX 事件分析
5. **Imputation**: 缺失值填补实验

**低优先级:**
6. **统计显著性测试**: 多种子运行 (seeds: 42, 123, 456, 789, 2024)
