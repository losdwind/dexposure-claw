...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/fig1_system_overview}
    \caption{System overview of DeXposure-Agent. The agent ingests weekly exposure snapshots, queries the DeXposure-FM backbone for multi-horizon forecasts, computes risk metrics and stress scenarios, and emits auditable recommendation tickets under data-health and confidence gates.}
...
\midrule
M1 & Systemic Importance & $\max_{p\in V} \mathrm{PageRank}(\widehat{G}_{t+h})_p$ \\
M3 & HHI Concentration & $\sum_p \bigl(d_p / \textstyle\sum_q d_q\bigr)^2$, \ $d_p = \text{weighted out-degree}$ \\
M4 & Network Density & $|E_{t+h}| \;/\; |V|(|V|-1)$ \\
M6 & PageRank Gini & $\mathrm{Gini}\!\bigl(\{\mathrm{PR}_p\}_{p\in V}\bigr)$ \\
M7 & Degree Gini & $\mathrm{Gini}\!\bigl(\{d_p\}_{p\in V}\bigr)$ \\
\bottomrule
\end{tabular}
...
\mathbf{M}_{t,h} := \Phi(\widehat{G}_{t+h},X_t)\in\mathbb{R}^5,\qquad
...
\mathcal{A}_t :=
\Bigl\{\, a=(h,j)\;:\; \frac{|M_{t,h}^{(j)} - \mu_t^{(j)}|}{\max(\sigma_t^{(j)},\,\epsilon)} > z \Bigr\},
\end{equation}
...
Each alert $a\in\mathcal{A}_t$ carries an evidence bundle $\mathrm{Ev}_t(a) = ( M_{t,h}^{(j)},\, \mu_t^{(j)},\, \sigma_t^{(j)},\, \mathrm{Attr}_t(a),\, h )$, where $\mathrm{Attr}_t(a)$ is a ranked list of top-$K$ contributing nodes/edges obtained via marginal contribution approximation: $\mathrm{Contrib}(e) := \Phi^{(j)}(\widehat{G}_{t+h}) - \Phi^{(j)}(\widehat{G}_{t+h}\setminus e)$ (default $K=10$).

\paragraph{Uncertainty-aware confidence.}
Alert reliability depends on three factors: data quality, forecast dispersion, and horizon length. Using Monte Carlo samples, we compute the coefficient of variation $\mathrm{CV}_{t,h}^{(j)} := \sigma(\{\widetilde{M}^{(m,j)}_{t,h}\}_m) / |\bar{M}^{(j)}_{t,h}|$ (clamped to $[0,1]$) and define:
\begin{equation}
...
L_{t,h}(s) := \frac{\sum_e w_e(\widehat{G}_{t+h}) - \sum_e w_e(\widehat{G}_{t+h}^s)}{\sum_e w_e(\widehat{G}_{t+h})},\quad
...
\texttt{Recommend-Reduce} & High & 0.75 & No & 2 \\
...
\begin{algorithm}[t]
\caption{DeXposure-Agent: one epoch}
...
\ForAll{$h\in\mathcal{H}$}
    ...
    \State $\mathbf{M}_{t,h} \gets \Phi(\widehat{G}_{t+h},X_t)$ \Comment{Table~\ref{tab:metrics}: M1,M3,M4,M6,M7}
    \State $\mathcal{A}_{t,h} \gets \{(h,j): |M^{(j)}_{t,h}-\mu^{(j)}_t|/\sigma^{(j)}_t > z\}$
    \State Attach $\mathrm{Ev}_t(a)$ and $C_t(a)$ for all $a\in\mathcal{A}_{t,h}$ \Comment{Eq.~\eqref{eq:alert_confidence}}
    \ForAll{$s\in\mathcal{S}_0$}
        \State $L_{t,h}(s) \gets \mathrm{StressLoss}(\widehat{G}_{t+h}, s)$;\; compute CVaR from $\{\widetilde{G}^{(m)}\}$
    \EndFor
\EndFor
...
\State $\mathcal{D}_t \gets \mathrm{PlaybookDecision}(\mathcal{A}_t,\mathcal{R}_t, DH_t,\mathrm{SAFE\_MODE}_t,\tau_{\mathrm{conf}})$ \Comment{Table~\ref{tab:playbook}}
