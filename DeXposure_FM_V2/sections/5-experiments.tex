...
(1) \textbf{GraphPFN}: a pre-trained GraphPFN encoder with frozen weights; only lightweight task-specific prediction heads are trained on DeXposure (a small MLP probe). In contrast, DeXposure-FM uses GraphPFN weights as initialization and fine-tunes the encoder and task heads end-to-end.
(2) \textbf{ROLAND}: a representative temporal graph neural network \citep{YouEtAl2022ROLAND}, trained from scratch.
(3) \textbf{Persistence}: a naive baseline that carries forward the last observed graph and edge weights ($\hat{A}_{t+h}=A_t$, $\hat{W}_{t+h}=W_t$).

For the trained baselines (GraphPFN and ROLAND), we use the same optimizer (Adam), learning rate ($5 \times 10^{-4}$), and training schedule as DeXposure-FM.
