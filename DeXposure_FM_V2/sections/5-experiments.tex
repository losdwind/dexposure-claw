...
\begin{enumerate}
  ...
  \item \textbf{Model value is in link prediction:} The primary value of DeXposure-FM lies in edge existence prediction. Compared to learned baselines, it achieves 3--8\% AUPRC improvement over GraphPFN-Frozen and 10--12\% over ROLAND, while maintaining comparable weight and node predictions.
...
\begin{enumerate}
  ...
  \end{equation}
  \item \textbf{Iterate:} If $\text{Loss}_q^{(r)} > \tau \cdot \text{TVL}_q$ (with distress threshold $\tau = 0.1$, i.e., 10\% TVL loss), protocol $q$ becomes distressed and propagates losses to its creditors.
