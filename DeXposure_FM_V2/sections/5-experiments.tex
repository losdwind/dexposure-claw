...
%\begin{enumerate}
%  \item 
(1) {GraphPFN:} pre-trained GraphPFN encoder with frozen weights; only task-specific prediction heads are fine-tuned on DeXposure (linear probe). In contrast, {DeXposure-FM} uses GraphPFN weight as initialization of the encoder and then fine-tunes the whole networks with customized heads; and (2)
...
We use DeXposure-FM trained on the pre-2025 training period and early-stopped on the pre-2025 validation period (Section~\ref{subsec:training-pipeline}). We then evaluate on the strict 2025 hold-out set without re-estimation during the test period. Each test week provides an origin $t$, yielding multiple out-of-sample pairs $(t,t+h)$ for horizons $h\in\{1,4,8,12\}$. To handle protocol entry/exit, all comparisons are computed on the common node set $V_t \cap V_{t+h}$.
