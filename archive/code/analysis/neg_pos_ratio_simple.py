...
def analyze_neg_pos_ratio(network_data: Dict) -> tuple:
    """
    分析所有快照的真实负正边比例
    ...
    for date, edges_data in network_data.items():
        ...
        num_edges = len(edges_data["links"])  # 数据使用 links 而不是 edges
        ...
        neg_pos_ratios.append(neg_pos_ratio)

    # 汇总统计
    n = len(neg_pos_ratios)
    neg_pos_ratios_sorted = sorted(neg_pos_ratios)
    densities_sorted = sorted(densities)

    summary = {
        "num_snapshots": len(ratios),
        "neg_pos_ratio_mean": sum(neg_pos_ratios) / n,
        "neg_pos_ratio_median": neg_pos_ratios_sorted[n // 2],
        "neg_pos_ratio_min": neg_pos_ratios_sorted[0],
        "neg_pos_ratio_max": neg_pos_ratios_sorted[-1],
        "density_mean": sum(densities) / n,
        "density_median": densities_sorted[n // 2],
        "density_min": densities_sorted[0],
        "density_max": densities_sorted[-1],
    }
...
def compare_with_config(summary: dict, config_neg_ratio: int = 5):
    ...
    print(f"  密度中位数:     {summary['density_median']:.6f}")
    print(
        f"  密度范围:       [{summary['density_min']:.6f}, {summary['density_max']:.6f}]"
    )

    print(f"\n与当前配置对比:")
    print(f"  当前负采样比例: {config_neg_ratio}:1")
    print(f"  真实负正比:     {summary['neg_pos_ratio_median']:.1f}:1")
    diff_ratio = summary["neg_pos_ratio_median"] / config_neg_ratio
    print(f"  差异倍数:       {diff_ratio:.1f}x")

    if diff_ratio > 20:
        print(f"\n⚠️  严重偏差: 真实负样本是当前配置的 {diff_ratio:.0f} 倍")
        print(f"   这会导致 AUPRC 显著虚高！")
    elif diff_ratio > 10:
        print(f"\n⚠️  严重偏差: 真实负样本是当前配置的 {diff_ratio:.0f} 倍")
        print(f"   这会导致 AUPRC 明显虚高")
    elif diff_ratio > 5:
        print(f"\n⚠️  中等偏差: 真实负样本是当前配置的 {diff_ratio:.1f} 倍")
        print(f"   建议: 调整负采样比例或使用 AUPRC 校正")
    else:
        print(f"\n✓ 负采样比例较为合理")


def recommend_sampling_strategy(summary: dict) -> dict:
    """
    根据数据特性推荐负采样策略
    """
    print("\n" + "=" * 60)
    ...
    print("\n策略 1: 渐进式负采样 (推荐)")
    train_ratios = [10, 20, 50]
    for r in train_ratios:
        print(f"  - 训练/验证: {r}:1, 测试: {min(200, int(real_ratio))}:1 (接近真实)")
    recommendations["progressive"] = {
        "train_neg_ratio": 20,
        "test_neg_ratio": min(200, int(real_ratio)),
    ...
    print(f"  - 使用公式: AUPRC_true ≈ AUPRC_pred * ({correction_factor:.2f})")
    print(f"  - 当前 0.93 AUPRC 校正后 ≈ {0.93 * correction_factor:.2f}")
