...
\hline
\end{tabular}
\caption{Multi-step forecasting results on 2025 hold-out test set. Bold indicates best performance within each horizon. FT = Fine-tuned, FR = Frozen (linear probe). Results from 1-epoch validation run.}
...
\paragraph{Results.}
Table~\ref{tab:imputation-results} summarizes imputation performance using GraphPFN (Finetuned).
...
\hline
\end{tabular}
\caption{Imputation performance at different masking rates using GraphPFN (Finetuned). Edge Recall measures the fraction of masked edges correctly reconstructed. Remarkably stable performance across masking rates suggests the model learns robust structural patterns.}
