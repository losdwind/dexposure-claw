...
& 1 & \textbf{0.995} & \textbf{0.972} & \textbf{2.465} & \textbf{3.420} & \textbf{0.058} & \textbf{0.400} \\
& 4 & \textbf{0.995} & \textbf{0.973} & \textbf{2.489} & \textbf{3.480} & \textbf{0.115} & \textbf{0.605} \\
& 8 & \textbf{0.994} & \textbf{0.967} & \textbf{2.554} & \textbf{3.509} & \textbf{0.229} & \textbf{0.890} \\
& 12 & \textbf{0.993} & \textbf{0.967} & \textbf{2.648} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
\hline
\multirow{4}{*}{GraphPFN}
& 1 & 0.990 & 0.947 & 3.260 & 4.260 & \textit{TBD} & \textit{TBD} \\
& 4 & 0.988 & 0.940 & 3.189 & 4.290 & \textit{TBD} & \textit{TBD} \\
& 8 & 0.987 & 0.938 & 3.169 & 4.250 & \textit{TBD} & \textit{TBD} \\
& 12 & 0.986 & 0.936 & 3.136 & 4.320 & \textit{TBD} & \textit{TBD} \\
\hline
\multirow{4}{*}{ROLAND}
& 1 & 0.961 & 0.865 & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
& 4 & 0.962 & 0.868 & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
& 8 & 0.961 & 0.867 & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
& 12 & 0.961 & 0.866 & \textit{TBD} & \textit{TBD} & \textit{TBD} & \textit{TBD} \\
\hline
\end{tabular}
\caption{Multi-step forecasting results on 2025 hold-out test set at horizons $h \in \{1,4,8,12\}$ weeks. DeXposure-FM = Fine-tuned, GraphPFN = Frozen (linear probe). \textit{TBD} indicates runs not completed yet.}
\label{tab:main-results}
\end{table}

\paragraph{Key findings.}
\begin{enumerate}
  \item \textbf{Edge existence:} DeXposure-FM achieves consistently high performance across horizons with AUROC 0.993--0.995 and AUPRC 0.967--0.973, outperforming GraphPFN (0.986--0.990/0.936--0.947) and ROLAND (0.961--0.962/0.865--0.868).
  \item \textbf{Edge weight:} DeXposure-FM shows substantial and stable improvement in weight prediction with MAE 2.465--2.648 across all horizons, vs.\ 3.136--3.260 (GraphPFN), a 18--24\% reduction.
  \item \textbf{Node prediction:} Node TVL change prediction degrades with horizon as expected: DeXposure-FM MAE increases from 0.058 ($h=1$) to 0.229 ($h=8$), reflecting growing uncertainty in longer-term forecasts.
  \item \textbf{Horizon stability:} DeXposure-FM maintains remarkably stable edge existence prediction (AUPRC 0.967--0.973) across all horizons up to 12 weeks, while GraphPFN shows gradual degradation (0.947 $\to$ 0.936) and ROLAND remains at lower levels (0.865--0.868).
\end{enumerate}
...
The 98.5\% mean edge overlap in DeFi networks means that predicting ``all edges persist'' achieves high accuracy. This makes absolute AUPRC/AUROC less informative than relative improvements. DeXposure-FM's 2.6\% AUPRC gain over GraphPFN at $h=1$ (0.972 vs.\ 0.947) represents meaningful improvement in identifying the 1.5\% of edges that do change.

\paragraph{Weight prediction benefits most from fine-tuning.}
The 16--24\% MAE reduction in edge weight prediction (2.465--2.648 vs.\ 3.136--3.260 for GraphPFN) demonstrates that fine-tuning captures domain-specific exposure dynamics that frozen encoders miss. The reduction is most pronounced at shorter horizons (24\% at $h=1$) and remains substantial even at 12 weeks (16\%), confirming the value of domain adaptation. This is critical for policy applications where exposure magnitudes determine systemic risk.
...
The contagion model assumes linear loss propagation and does not capture feedback effects (e.g., liquidation cascades). Node TVL predictions degrade at longer horizons (MAE 0.058 at $h=1$ vs.\ 0.229 at $h=8$), reflecting inherent uncertainty in protocol-level forecasts beyond 2--3 months. The current $h=12$ experiment used validation windows insufficient for early stopping (Val=0 due to the 12-week horizon matching the 12-week validation period), requiring all 20 epochs; future work will use 24-week validation windows to enable proper model selection at longer horizons.
